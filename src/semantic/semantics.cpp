...
namespace cf
{
    ...
    {
        if (auto *d = dynamic_cast<StmtDecl *>(&s))
            return check_decl(*d);
        if (auto *a = dynamic_cast<StmtAssign *>(&s))
            return check_assign(*a);
        if (auto *pr = dynamic_cast<StmtPrint *>(&s))
            return check_print(*pr);
        if (auto *w = dynamic_cast<StmtWhile *>(&s))
            return check_while(*w);
        if (auto *i = dynamic_cast<StmtIf *>(&s))
            return check_if(*i);
        if (auto *f = dynamic_cast<StmtFor *>(&s))
            return check_for(*f);
     ...
     *
     * - Para cada argumento:
     *  - Verifica o tipo da expressão (check_expr).
     * - Aceita tipos Inteiro, Logico, Caractere.
     * - Aceita literal string (ExprString).
     * - Aceita expressões que contenham strings (concatenações).
    ...
    void Semantic::check_print(StmtPrint &s)
    {
        for (auto &e : s.args)
        {
            ...
            {
                // concatenações de string são aceitas como texto para Imprimir
                continue;
            }
     ...
     * - Dependendo do operador (e.op):
     *   - Aritméticos (+, -, *, /, %, ^): usa result_arith; se inválido, erro.
     *   - Relacionais (==, !=, >, <, >=, <=): usa result_rel; se inválido, erro.
     *   - Lógicos (&&, ||): usa result_logic; se inválido, erro.
     *  - Para o operador '+', se qualquer lado for string (literal ou sub-expressão contendo string),
     * tratamos como concatenação textual.
     *    - Define e retorna o tipo inferido da expressão (e.inferred).
     *  - Retorna o tipo inferido da expressão (e.inferred).
     */
    CfType Semantic::check_binary(ExprBinary &e)
    {
        ...
        switch (e.op)
        {
        // '+' pode ser aritmético OU concatenação
        case BinOp::Add:
        {
            // 🔹 Se qualquer lado contiver string, tratamos como concatenação textual
            if (expr_has_string(*e.lhs) || expr_has_string(*e.rhs))
            {
                // Não participamos de aritmética nem de relações numéricas,
                // mas deixamos a expressão "valer" para Imprimir.
                e.inferred = CfType::Desconhecido;
                return e.inferred;
            }

            // Caso contrário, é aritmético normal (como já era antes)
    ...
    /**
     * Verifica se a expressão (ou alguma sub-expressão) é uma string.
     *
     * - Retorna true se a expressão for um literal string, ou se for um grupo ou binária
     *   que contenha uma string em algum lugar.
     * - Retorna false caso contrário.
     */
    bool Semantic::expr_has_string(Expr &e)
    {
        // Se for um literal string diretamente
        if (dynamic_cast<ExprString *>(&e))
        {
            return true;
        }

        // Se for um grupo (parênteses), olha expressão interna
        if (auto *g = dynamic_cast<ExprGroup *>(&e))
        {
            return expr_has_string(*g->inner);
        }

        // Se for expressão binária, olha os filhos
        if (auto *b = dynamic_cast<ExprBinary *>(&e))
        {
            return expr_has_string(*b->lhs) || expr_has_string(*b->rhs);
        }

        // Demais tipos: não são string nem contêm string
        return false;
    }
