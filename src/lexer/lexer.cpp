...
namespace cf {
    ...
    Token Lexer::lex_string(){
        std::string lx; get(); // abre "
        while (true){
            char c = peekc();
            if (c == '\0'){
                Diagnostic d; d.phase="lexical"; d.pos=pos_; d.message="string não terminada";
                d.line_text = current_line_text(); throw CompileError(d);
            }
            if (c == '"'){ get(); break; }
            if (c == '\\'){
                lx.push_back(get()); // '\\'
                char n = peekc();
                if (n == '\0'){
                    Diagnostic d; d.phase="lexical"; d.pos=pos_; d.message="escape inválido no final de string";
                    d.line_text = current_line_text(); throw CompileError(d);
                }
    ...
    Token Lexer::lex_char(){
        std::string content; get(); // abre '
        char c = peekc();
        if (c == '\0'){
            Diagnostic d; d.phase="lexical"; d.pos=pos_; d.message="caractere não terminado";
            d.line_text = current_line_text(); throw CompileError(d);
        }
        if (c == '\\'){ // escape
            ...
            char e = peekc();
            if (e == '\0' || e == '\n'){
                Diagnostic d; d.phase="lexical"; d.pos=pos_; d.message="escape inválido em caractere";
                d.line_text = current_line_text(); throw CompileError(d);
            }
            content.push_back(get());
        } else {
        ...
        if (peekc() != '\''){
            Diagnostic d; d.phase="lexical"; d.pos=pos_; d.message="caractere deve conter exatamente 1 símbolo";
            d.line_text = current_line_text(); throw CompileError(d);
        }
        get(); // fecha '
    ...
    Token Lexer::lex_token(){
        ...
        if (starts_with("<-")){ get(); get(); return make(TokenKind::Assign, "<-"); }
        if (starts_with("**")){ get(); get(); return make(TokenKind::Pow, "**"); }
        if (starts_with("<>")){ get(); get(); return make(TokenKind::Ne, "<>"); }
        if (starts_with(">=")){ get(); get(); return make(TokenKind::Ge, ">="); }
        if (starts_with("<=")){ get(); get(); return make(TokenKind::Le, "<="); }
